...
float filmDirt(vec2 pp, float time) 
{
	...
	vec3 nse2 =
		textureLod(Texture1,.1*nseLookup2.xy,0.).xyz +
		textureLod(Texture1,.01*nseLookup2.xy,0.).xyz +
		textureLod(Texture1,.004*nseLookup2.xy+0.4,0.).xyz;
	float thresh = .6;
	...
	float seed = textureLod(Texture1,vec2(time*.35,time),0.).x;
...
void main()
{
	...
	float time = limitFPS(Time, FPS);

	vec4 jumpCutData = jumpCut(time);
    vec4 dirt = vec4(filmDirt(qq, time + jumpCutData.w));     
    vec4 image = texture(Texture0, moveImage(uv, time));   
    vec4 vig = vignette(uv, time);
    
    FinalColor = image * dirt * vig;
    FinalColor = blackAndWhite(FinalColor);
}