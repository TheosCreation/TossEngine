...
// Add Title string of Dialog
void ImGui::SetInputTextTitle(const ImWchar *title, ImGuiID id)
{
    IM_ASSERT(title != nullptr);
    if (title != nullptr)
    {
        GImGui->DialogTitle.SetVoidPtr(id, (void*)title);
    }
}

void ImGui::SetInputTextTitle(const ImWchar *title, const char *label, ImGuiWindow *window/* = nullptr*/)
{
    IM_ASSERT(label != nullptr);
    if (label != nullptr)
    {
        if (window == nullptr) window = ImGui::GetCurrentWindow();
        SetInputTextTitle(title, window->GetID(label));
    }
}

// Add Placeholder string of Dialog
void ImGui::SetInputTextPlaceholder(const ImWchar *placeholder, ImGuiID id)
{
    IM_ASSERT(placeholder != nullptr);
    if (placeholder != nullptr)
    {
        GImGui->DialogPlaceholder.SetVoidPtr(id, (void*)placeholder);
    }
}

void ImGui::SetInputTextPlaceholder(const ImWchar *placeholder, const char *label, ImGuiWindow *window/* = nullptr*/)
{
    IM_ASSERT(label != nullptr);
    if (label != nullptr)
    {
        if (window == nullptr) window = ImGui::GetCurrentWindow();
        SetInputTextPlaceholder(placeholder, window->GetID(label));
    }
}

// Get Title string for Dialog
const ImWchar *ImGui::GetInputTextTitle(ImGuiID id)
{
    return (ImWchar*)GImGui->DialogTitle.GetVoidPtr(id);
}

const ImWchar *ImGui::GetInputTextTitle(const char *label, ImGuiWindow *window/* = nullptr*/)
{
    if (window == nullptr) window = ImGui::GetCurrentWindow();
    return (ImWchar*)GImGui->DialogTitle.GetVoidPtr(window->GetID(label));
}

// Get Plcaeholder string for Dialog
const ImWchar *ImGui::GetInputTextPlaceholder(ImGuiID id)
{
    return (ImWchar*)GImGui->DialogPlaceholder.GetVoidPtr(id);
}

const ImWchar *ImGui::GetInputTextPlaceholder(const char *label, ImGuiWindow *window/* = nullptr*/)
{
    if (window == nullptr) window = ImGui::GetCurrentWindow();
    return (ImWchar*)GImGui->DialogPlaceholder.GetVoidPtr(window->GetID(label));
}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (hovered)
    {
        ...
        g.Style.MouseCursorScale = size.y / 16.0f;  // Size of ImGuiMouseCursor_TextInput is ImVec2(7, 16)
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        // Keep for dialog setup
        edit_state.DialogUserFlags = flags;
