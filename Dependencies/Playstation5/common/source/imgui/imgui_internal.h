...
#define WINDOW_NAME_DEBUG_STRING		"Debug##DebugStrings"		// Window name for debugString
#define IMGUI_FLAG_FOR_DEBUG_STRING		(ImGuiWindowFlags_NoBackground|ImGuiWindowFlags_NoNav|ImGuiWindowFlags_NoDecoration|ImGuiWindowFlags_NoInputs)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    // Use to setup IME dialog
    ImGuiInputTextFlags     DialogUserFlags;
...
struct ImGuiContext
{
    ...
    char                    TempBuffer[1024*16+1];               // Temporary text buffer

    // Flag for drawing objects with depth test.
    // If true,  draw ImGui object with depth value which you set.
    // if false, draw ImGui object with depth == 0.0f. (default)
    bool                    EnableDepth;

	// String data for Dialog
	ImGuiStorage			DialogTitle;
	ImGuiStorage			DialogPlaceholder;

	// ImGui is used only "drawDebugString"
	bool					UsedOnlyDebugString;
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : OverlayDrawList(NULL)
    {
        ...
        EnableDepth = false;
		UsedOnlyDebugString = true;	// This is updated in ImGui::Begin.
    }
};
