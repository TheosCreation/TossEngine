...
// SIE customization
void ImGui::SetEnableDepth(bool enabled)
{
    GImGui->EnableDepth = enabled;
}
...
void ImGui::NewFrame()
{
    ...
    SetNextWindowSize(g.IO.DisplaySize, ImGuiCond_FirstUseEver);
    Begin(WINDOW_NAME_DEBUG_STRING, 0, IMGUI_FLAG_FOR_DEBUG_STRING);
...
void ImGui::Render()
{
    ...
    if (g.IO.MouseDrawCursor)
    {
        ...
        {
            // Scalling by ScreenSize: about 8K=3.2f, 4K=2.2f, 2k=1.7f;
            g.Style.MouseCursorScale = 2.2f + (GetIO().DisplaySize.y - 2160.0f) / 2160.0f;
        }
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    // Is this used for other than "drawDebugString" ?
    if (0 != strncmp(name, WINDOW_NAME_DEBUG_STRING, 512))
    {
        g.UsedOnlyDebugString = false;
    }
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            {
                //changed: added branch for focused window
                ImGuiCol guicol = GetWindowBgColorIdxFromFlags(flags);
                if ((guicol == ImGuiCol_WindowBg) && title_bar_is_highlight)
                {
                    guicol = ImGuiCol_WindowBgActive;
                }
                ImU32 bg_col = GetColorU32(guicol);
                if (g.NextWindowData.BgAlphaCond != 0)
                    bg_col = (bg_col & ~IM_COL32_A_MASK) | ((ImU32)(IM_F32_TO_INT8_SAT(g.NextWindowData.BgAlphaVal) << IM_COL32_A_SHIFT));
...
{
    //added:
    // Push default WindowBgActive with WindowBg.
    // So, set "real" WindowBgActive after WindowBg pushed.
    if (idx == ImGuiCol_WindowBg)
    {
        PushStyleColor(ImGuiCol_WindowBgActive, col);
    }
...
void ImGui::PopStyleColor(int count)
{
    ...
    while (count > 0)
    {
        ...
        count--;
        //added:
        // Pop WindowBgActive setted as default with WindowBg.
        if (backup.Col == ImGuiCol_WindowBg)
        {
            PopStyleColor();
        }
    }
...
ImVec2 ImGui::GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources, ImGuiInputReadMode mode, float slow_factor, float fast_factor)
{
    ...
    if (slow_factor != 0.0f && IsNavInputDown(ImGuiNavInput_TweakSlow))
        delta *= (slow_factor * (1.0f / (GImGui->IO.NavInputs[ImGuiNavInput_TweakSlow] * 0.9f + 0.1f)));
    if (fast_factor != 0.0f && IsNavInputDown(ImGuiNavInput_TweakFast))
        delta *= (fast_factor * GImGui->IO.NavInputs[ImGuiNavInput_TweakFast] * 0.9f + 1.0f);
    return delta;
}
