...
static bool pfxContactTriangleCylinder(PfxContactCache &contacts,PfxUInt32 facetId,
							const PfxVector3 &normal,const PfxVector3 &p0,const PfxVector3 &p1,const PfxVector3 &p2,
							const PfxFloat thickness,const PfxVector3 &edgeAngle,PfxUInt32 edgeChk,const PfxCylinder &cylinder)
{
	// 早期チェック + 裏側の誤判定を防ぐ
	PfxVector3 sA(0.0f);
	getSupportVertex(&cylinder, normal, sA, 0.0f);
	...
	PfxMpr<PfxFatTriangle, PfxCylinder> collider(&fatTriangle, &cylinder);
...
template<>
PfxInt32 pfxContactTriMesh<PfxCylinder, PfxExpandedTriMesh>(
	...
	PfxFloat distanceThreshold)
{
	...
	for(PfxUInt32 f = 0; f < numSelFacets; f++) {
		...
		pfxContactTriangleCylinder(localContacts,selFacets[f],
							...
							facet.m_thickness,edgeAngle,edgeChk,cylinderB);
	}
...
template<>
PfxInt32 pfxContactTriMesh<PfxCylinder, PfxQuantizedTriMeshBvh>(
	PfxContactCache &contacts,
	...
	PfxFloat distanceThreshold )
{
	...
	for(PfxUInt32 f = 0; f < numSelFacets; f++ ) {
		...
		pfxContactTriangleCylinder(localContacts, selFacets[f],
							facetNormal,facetPntsA[0],facetPntsA[1],facetPntsA[2],
							decodedFacet.m_thickness,edgeAngle,edgeChk,cylinderB);
	}
...
template<>
PfxInt32 pfxContactTriMesh<PfxCylinder, PfxCompressedTriMesh>(
	PfxContactCache &contacts,
	...
	PfxFloat distanceThreshold)
{
	...
	for(PfxUInt32 f = 0; f < numSelFacets; f++ ) {
		...
		if(facet.m_facetInfo & 0x8000) {
			...
			pfxContactTriangleCylinder(localContacts, facetId0,
								facetNormal0,facetPntsA[0],facetPntsA[1],facetPntsA[2],
								largeMeshA->m_defaultThickness,edgeAngle,edgeChk0,cylinderB);

			pfxContactTriangleCylinder(localContacts, facetId1,
								facetNormal1,facetPntsA[0],facetPntsA[2],facetPntsA[3],
								largeMeshA->m_defaultThickness,edgeAngle,edgeChk1,cylinderB);
		}
		else {
			// signle facet
			...
			pfxContactTriangleCylinder(localContacts, selFacets[f],
								facetNormal,facetPntsA[0],facetPntsA[1],facetPntsA[2],
								largeMeshA->m_defaultThickness,edgeAngle,edgeChk,cylinderB);
		}
	}
