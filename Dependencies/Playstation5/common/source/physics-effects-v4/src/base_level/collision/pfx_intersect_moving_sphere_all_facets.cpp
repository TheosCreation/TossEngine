...
#include "pfx_intersect_moving_sphere_all_facets.h"
#include <float.h>
...
PfxBool pfxIntersectMovingSphereTriMesh(
	...
	pfxSphereHitCallback callback, void *userData)
{
	(void) largeMesh;
	...
	for(PfxUInt32 f = 0; f < numSelFacets; f++) {
		...
		//  only checking the ray direction against the normal will make test fail if the sphere collides with the triangle
		if(dot(normal,sphereIn.m_startPosition-triangle.points[ 0 ]) <= 0.0f) {	//  
			continue;											//  
		}														//  
		
		PfxVector3 facetAabbMin = minPerElem(minPerElem(triangle.points[0],triangle.points[1]),triangle.points[2]) - PfxVector3(sphereIn.m_radius);
		PfxVector3 facetAabbMax = maxPerElem(maxPerElem(triangle.points[0],triangle.points[1]),triangle.points[2]) + PfxVector3(sphereIn.m_radius);
		
		PfxFloat cur_t = 1.0f;
		
		if( !pfxIntersectRayAABBFast(sphereIn.m_startPosition,sphereIn.m_direction,0.5f * (facetAabbMax + facetAabbMin),0.5f * (facetAabbMax - facetAabbMin),cur_t) )
			continue;
		...
		PfxFloat tmpSquaredDistance(sphereIn.m_radius*sphereIn.m_radius);																// 
		if(pfxIntersectMovingSphereTriangle(sphereIn.m_startPosition,sphereIn.m_direction,sphereIn.m_radius,triangle,cur_t,tmpSquaredDistance)) {        // 
			PfxVector3 stopPoint = sphereIn.m_startPosition + cur_t * sphereIn.m_direction;
			...
			hit.m_contactPoint.segment = sphereIn.m_segment;
	...
	while(!bvhStack.empty()) {
		...
		for(PfxUInt32 f=0;f<numFacets;f++) {
			...
			if(facet.m_facetInfo & 0x8000) {
				...
				//  only checking the ray direction against the normal will make test fail if the sphere collides with the triangle
				if(dot(normal1,sphereIn.m_startPosition-triangle1.points[ 0 ]) > 0.0f) { //  
					//  The distance between sphere and triangle will be equal to radius if the sphere touches the triangle and between [0,radius] if the sphere penetrates the triangle (which
					// is only possible if the sphere intersects at t=0). If we intersect at t=0 we want to report the contact with deepest penetration.
					PfxFloat tmpSquaredDistance(sphereIn.m_radius*sphereIn.m_radius);																// 
					if(pfxIntersectMovingSphereTriangle(sphereIn.m_startPosition,sphereIn.m_direction,sphereIn.m_radius,triangle1,cur_t,tmpSquaredDistance)) {        // 
						PfxVector3 stopPoint = sphereIn.m_startPosition + cur_t * sphereIn.m_direction;
				...
				//  only checking the ray direction against the normal will make test fail if the sphere collides with the triangle
				if(dot(normal2,sphereIn.m_startPosition-triangle2.points[ 0 ]) > 0.0f) { //  
					//  The distance between sphere and triangle will be equal to radius if the sphere touches the triangle and between [0,radius] if the sphere penetrates the triangle (which
					// is only possible if the sphere intersects at t=0). If we intersect at t=0 we want to report the contact with deepest penetration.
					PfxFloat tmpSquaredDistance(sphereIn.m_radius*sphereIn.m_radius);																// 
					if(pfxIntersectMovingSphereTriangle(sphereIn.m_startPosition,sphereIn.m_direction,sphereIn.m_radius,triangle2,cur_t,tmpSquaredDistance)) {        // 
						PfxVector3 stopPoint = sphereIn.m_startPosition + cur_t * sphereIn.m_direction;
				...
				PfxVector3 normal = triangle.calcNormal();
				//PfxFloat angle = dot(normal,sphereIn.m_direction);
...
PfxBool pfxIntersectMovingSphereAllFacetsInLargeTriMeshBvh(const PfxSphereInputInternal &sphereIn,
	pfxSphereHitCallback callback, void *userData,
	const PfxLargeTriMeshImpl *largeMesh,const PfxTransform3 &transform,PfxBool flipTriangle,PfxFloat radiusLocal)
{
	...
	PfxVector3 startPosLocal = transformLMesh.getUpper3x3() * sphereIn.m_startPosition + transformLMesh.getTranslation();
	PfxVector3 rayDirLocal = transformLMesh.getUpper3x3() * sphereIn.m_direction;
		...
		while( ! bvhStack.empty() ) {
			...
			for(PfxUInt32 i=0;i<numSelIslands;i++) {
				...
				if(largeMesh->m_type == SCE_PFX_LARGE_MESH_TYPE_HIGH_COMPRESSION) {
					...
					PfxBool ret = pfxIntersectMovingSphereTriMesh(
						largeMesh,(PfxCompressedTriMesh*)island,
						startPosLocal,rayDirLocal,radiusLocal, // Moving sphere in the mesh local coordinates
						sphereIn, // Moving sphere in the world coordinates
...
PfxBool pfxIntersectMovingSphereAllFacetsInLargeTriMeshArray(const PfxSphereInputInternal &sphereIn,
	pfxSphereHitCallback callback, void *userData,
	const PfxLargeTriMeshImpl *largeMesh,const PfxTransform3 &transform,PfxBool flipTriangle,PfxFloat radiusLocal)
{
	// レイをラージメッシュのローカル座標へ変換
			...
			for (PfxUInt32 i = 0; i<numSelIslands; i++) {
				...
				if (largeMesh->m_type == SCE_PFX_LARGE_MESH_TYPE_HIGH_COMPRESSION) {
					...
					PfxBool ret = pfxIntersectMovingSphereTriMesh(
						...
						sphereIn, // Moving sphere in the world coordinates
						transform, flipTriangle, islandId, callback, userData,
						discardTriangleCallback, userDataForDiscardingTriangle);
					if (!ret) return ret;
				}
				else if (largeMesh->m_type & 0x01) {
					PfxQuantizedTriMeshBvh *island = (PfxQuantizedTriMeshBvh*)largeMesh->m_islands + islandId;
	...
	void *userDataForDiscardingTriangle)
{
	...
	if(largeMesh->isUsingBvh()) {
		if(discardTriangleCallback)
			return pfxIntersectMovingSphereAllFacetsInLargeTriMeshBvh( sphereIn, callback, userData, largeMesh, transform, flipTriangle, radiusLocal, discardTriangleCallback, userDataForDiscardingTriangle);

		else 
			return pfxIntersectMovingSphereAllFacetsInLargeTriMeshBvh(sphereIn, callback, userData, largeMesh, transform, flipTriangle, radiusLocal);
	}
	
	if(discardTriangleCallback)
		return pfxIntersectMovingSphereAllFacetsInLargeTriMeshArray( sphereIn, callback, userData, largeMesh, transform, flipTriangle, radiusLocal, discardTriangleCallback, userDataForDiscardingTriangle);

	else 
		return pfxIntersectMovingSphereAllFacetsInLargeTriMeshArray(sphereIn, callback, userData, largeMesh, transform, flipTriangle, radiusLocal);
}
