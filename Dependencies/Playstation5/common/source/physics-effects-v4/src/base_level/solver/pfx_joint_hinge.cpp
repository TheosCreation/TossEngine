	...
	float lowerAngle, float upperAngle)
{
	...
	if(lowerAngle > upperAngle || 
		!SCE_PFX_RANGE_CHECK(lowerAngle,-SCE_PFX_PI-0.01f,SCE_PFX_PI+0.01f) || 
		!SCE_PFX_RANGE_CHECK(upperAngle,-SCE_PFX_PI-0.01f,SCE_PFX_PI+0.01f)) {
		return SCE_PFX_ERR_OUT_OF_RANGE;
	}
	joint.m_constraints[3].m_movableLowerLimit = lowerAngle;
	joint.m_constraints[3].m_movableUpperLimit = upperAngle;
...
{
	PfxInt32 ret = pfxInitializeHingeJointCommon(joint, stateA, stateB, param.damping, param.bias, param.lowerAngle, param.upperAngle);
	if(ret != SCE_PFX_OK)
		return ret;
	...
	)
{
	...
	PfxVector3 axis[3];
	PfxFloat angle[3];
	pfxCalcJointAngleHinge(worldFrameA, worldFrameB, angle, axis);
...
void pfxComputeHingeJointError( 
	PfxJoint &joint,            
	...
	float( &out_error )[ 6 ] )
{
	...
	for( int c = 0; c < 3; c++ ) {
		...
		out_error[ c ] = dot( distance, -normal );
	}
	...
	// Angular Constraint
	// constraint 3 is the rotation axis of the hinge joint
	// constraint 4 is the swing axis of the hinge joint (always locked)
	for( int c = 3; c < 6; c++ ) {
		PfxJointConstraint &jointConstraint = joint.m_constraints[ c ];
...
PfxFloat pfxComputeHingeJointAngle(
	PfxJoint &joint,
	const PfxRigidState &stateA,
	const PfxRigidState &stateB)
{
...
void pfxHingeJointGetAngleLimit(
	const PfxJoint &joint,
	PfxFloat &lowerAngle,
	PfxFloat &upperAngle)
{
	SCE_PFX_ASSERT(joint.m_type == kPfxJointHinge);
	lowerAngle = joint.m_constraints[3].m_movableLowerLimit;
	upperAngle = joint.m_constraints[3].m_movableUpperLimit;
}

PfxInt32 pfxHingeJointSetAngleLimit(
	PfxJoint &joint,
	PfxFloat lowerAngle,
	PfxFloat upperAngle)
{
	...
	)
{
	...
	for(int c=0;c<3;c++) {
		...
		constraint.m_rhs -= (jointConstraint.m_velocityBias * (-posErr)) / timeStep;
	...
	for(int c=3;c<6;c++) {
		...
		if(denom > 0.0f) {
			...
			constraint.m_rhs -= (jointConstraint.m_velocityBias * (-posErr)) / timeStep; // position error
			constraint.m_rhs *= jacDiagInv;
			constraint.m_jacDiagInv = jacDiagInv;
		}
		else {
