	...
	float swingLowerAngle, float swingUpperAngle )
{
	...
	if(swingLowerAngle > swingUpperAngle || 
		!SCE_PFX_RANGE_CHECK(swingLowerAngle,0.0f,SCE_PFX_PI+0.01f) || 
		!SCE_PFX_RANGE_CHECK(swingUpperAngle,0.0f,SCE_PFX_PI+0.01f)) {
		return SCE_PFX_ERR_OUT_OF_RANGE;
	}
	joint.m_constraints[4].m_movableLowerLimit = swingLowerAngle;
	joint.m_constraints[4].m_movableUpperLimit = swingUpperAngle;
...
{
	PfxInt32 ret = pfxInitializeSwingTwistJointCommon(joint, stateA, stateB, param.bias, param.damping, 
													  param.twistLowerAngle, param.twistUpperAngle, 
													  param.swingLowerAngle, param.swingUpperAngle);
	if( ret != SCE_PFX_OK )
		return ret;
	...
	)
{
	...
	for(int c=0;c<3;c++) {
		...
		if (fabsf(denom - jointConstraint.m_jacobian) > SCE_PFX_JOINT_WARM_STARTING_LINEAR_THRESHOLD) {
	...
	)
{
	...
	pfxSolveAngularConstraintRow(joint.m_constraints[3].m_constraintRow,
		joint.m_constraints[3].m_lowerLimit, joint.m_constraints[3].m_upperLimit,
	...
	pfxSolveAngularConstraintRow(joint.m_constraints[4].m_constraintRow,
		joint.m_constraints[4].m_lowerLimit, joint.m_constraints[4].m_upperLimit,
		solverBodyA.m_deltaAngularVelocity,solverBodyA.m_inertiaInv,rA,
		solverBodyB.m_deltaAngularVelocity,solverBodyB.m_inertiaInv,rB);

	pfxSolveAngularConstraintRow(joint.m_constraints[5].m_constraintRow,
		joint.m_constraints[5].m_lowerLimit, joint.m_constraints[5].m_upperLimit,
		solverBodyA.m_deltaAngularVelocity,solverBodyA.m_inertiaInv,rA,
		solverBodyB.m_deltaAngularVelocity,solverBodyB.m_inertiaInv,rB);
...
void pfxSwingTwistJointGetAngleLimit(
...
{
	SCE_PFX_ASSERT(joint.m_type == kPfxJointSwingTwist);
	twistLowerAngle = joint.m_constraints[3].m_movableLowerLimit;
	twistUpperAngle = joint.m_constraints[3].m_movableUpperLimit;
	swingLowerAngle = joint.m_constraints[4].m_movableLowerLimit;
	swingUpperAngle = joint.m_constraints[4].m_movableUpperLimit;
}

PfxInt32 pfxSwingTwistJointSetAngleLimit(
	PfxJoint &joint,
	PfxFloat twistLowerAngle,
	PfxFloat twistUpperAngle,
	PfxFloat swingLowerAngle,
...
void pfxComputeSwingTwistJointError(
	PfxJoint &joint,
	const PfxRigidState &stateA,
	const PfxRigidState &stateB,
	float( &out_error )[ 6 ]
