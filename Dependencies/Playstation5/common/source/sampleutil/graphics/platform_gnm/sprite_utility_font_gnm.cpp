...
namespace
{
	uint32_t	s_renderTargetWidth		= 1920;
	uint32_t	s_renderTargetHeight	= 1080;
...
namespace sce {	namespace SampleUtil { namespace Graphics {
	namespace SpriteUtil {
		...
		{
			SCE_SAMPLE_UTIL_ASSERT(width > 0 && width <= 3840 && height > 0 && height <= 2160);
		...
		)
		{
			...
			// Create font atlas includes only characters in "ucs2Charcode"
		...
		int	drawDebugString(
			sce::Vectormath::Simd::Aos::Vector2_arg	position,		// viewport coordinate
			float	fontHeight,
			sce::Vectormath::Simd::Aos::Vector4_arg	colorCoeff,
