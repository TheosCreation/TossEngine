...
static ssi::PadData convert(const ScePadVrControllerData &vrController)
{
	ssi::PadData ctrl;
	memset(&ctrl, 0, sizeof(ctrl));
	ctrl.connected = vrController.connected;

	if (!vrController.connected) {
		ctrl.lx = 0x80;
		ctrl.ly = 0x80;
		ctrl.rx = 0x80;
		ctrl.ry = 0x80;
	...
	ctrl.timeStamp = vrController.timestamp;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_L3    ) ? ssi::kButtonL3       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_R3    ) ? ssi::kButtonR3       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_OPTIONS ) ? ssi::kButtonOptions  : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_L2    ) ? ssi::kButtonL2       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_R2    ) ? ssi::kButtonR2       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_L1    ) ? ssi::kButtonL1       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_R1    ) ? ssi::kButtonR1       : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_TRIANGLE ) ? ssi::kButtonTriangle : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_CIRCLE   ) ? ssi::kButtonCircle   : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_CROSS    ) ? ssi::kButtonCross    : 0;
	ctrl.buttons |= (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_SQUARE   ) ? ssi::kButtonSquare   : 0;
	ctrl.lx = vrController.analogStick.x;
	ctrl.ly	= vrController.analogStick.y;
	ctrl.rx	= vrController.analogStick.x;
	ctrl.ry = vrController.analogStick.y;

	ctrl.l2	= vrController.analogButtons.l2;
	ctrl.r2 = vrController.analogButtons.r2;

	ctrl.motionData.acceleration.setX(vrController.acceleration.x);
	ctrl.motionData.acceleration.setY(vrController.acceleration.y);
	ctrl.motionData.acceleration.setZ(vrController.acceleration.z);
	ctrl.motionData.angularVelocity.setX(vrController.angularVelocity.x);
	ctrl.motionData.angularVelocity.setY(vrController.angularVelocity.y);
	ctrl.motionData.angularVelocity.setZ(vrController.angularVelocity.z);
	ctrl.motionData.orientation.setW(vrController.orientation.w);
	ctrl.motionData.orientation.setX(vrController.orientation.x);
	ctrl.motionData.orientation.setY(vrController.orientation.y);
	ctrl.motionData.orientation.setZ(vrController.orientation.z);

	ctrl.intercepted = (vrController.buttons & SCE_PAD_VR_CONTROLLER_BUTTON_INTERCEPTED) != 0;
	ctrl.connectedCount = vrController.connectedCount;
...
int ssi::PadContext::initialize(System::UserId userId, int32_t type, int32_t index, const ssi::PadContextOption* option)
{
	...
	const bool isVrController = (m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_LEFT || m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_RIGHT);
	...
	const uint8_t minimumDeadZone = 1;
	...
	if (!isVrController)
	{
		...
		m_defaultRightAnalogStickDeadZone = (float)(std::max(info.stickInfo.deadZoneRight, minimumDeadZone) * 2) * m_recipMaxByteAsFloat;
		m_defaultLeftAnalogStickDeadZone = (float)(std::max(info.stickInfo.deadZoneLeft, minimumDeadZone) * 2) * m_recipMaxByteAsFloat;
		m_touchPadWidth = info.touchPadInfo.resolution.x;
		m_touchPadHeight = info.touchPadInfo.resolution.y;
		m_touchPadPixelDensity = info.touchPadInfo.pixelDensity;
	} else {
		ScePadVrControllerDeviceInformation info;
		ret = scePadVrControllerGetDeviceInformation(m_handle, &info);
		SCE_SAMPLE_UTIL_ASSERT_EQUAL(ret, SCE_OK);

		m_defaultRightAnalogStickDeadZone = (float)(std::max(info.stickDeadZone, minimumDeadZone) * 2) * m_recipMaxByteAsFloat;
		m_defaultLeftAnalogStickDeadZone = (float)(std::max(info.stickDeadZone, minimumDeadZone) * 2) * m_recipMaxByteAsFloat;
		m_touchPadWidth = 0;
		m_touchPadHeight = 0;
		m_touchPadPixelDensity = 0.f;
	}
	m_connectionType       = -1;

	if (m_touchPadHeight != 0)
	{
		m_touchPadAspectRatio = (float)m_touchPadWidth / m_touchPadHeight;
	} else {
		m_touchPadAspectRatio = 1.f;
	}
	
	m_currentConnectedCount = 0;

	m_isRightAnalogStickDeadZoneUpdated = false;
	m_isLeftAnalogStickDeadZoneUpdated = false;
	...
	const size_t dataSize = (isVrController ? sizeof(ScePadVrControllerData) : sizeof(ScePadData)) * m_numPadData;
	m_currentData = malloc(dataSize);
	if (m_currentData == nullptr) {
		return SCE_SAMPLE_UTIL_ERROR_FATAL;
	}
	memset(m_currentData, 0, dataSize);
...
int ssi::PadContext::updatePadData(Input::PadData	*pScriptPadData)
{
	...
	if (m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_LEFT || m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_RIGHT)
	{
		ret = scePadVrControllerRead(m_handle, (ScePadVrControllerData*)m_currentData, m_numPadData);
		if (((ScePadVrControllerData*)m_currentData)->buttons & SCE_PAD_VR_CONTROLLER_BUTTON_INTERCEPTED)
		{
			memset(m_currentData, 0, sizeof(ScePadVrControllerData) * m_numPadData);
		}
		...
		} else
		{
			...
			if (((ScePadVrControllerData*)m_currentData)->connected && ((ScePadVrControllerData*)m_currentData)->connectedCount > m_currentConnectedCount)
			{
				ScePadVrControllerDeviceInformation info;
				int ret2 = scePadVrControllerGetDeviceInformation(m_handle, &info);
				...
				} else
				{
					...
					m_defaultRightAnalogStickDeadZone	= (float)(info.stickDeadZone * 2) * m_recipMaxByteAsFloat;
					m_defaultLeftAnalogStickDeadZone	= (float)(info.stickDeadZone * 2) * m_recipMaxByteAsFloat;
					if (!m_isRightAnalogStickDeadZoneUpdated)
					{
						m_rightAnalogStickDeadZone = m_defaultRightAnalogStickDeadZone;
					}
					if (!m_isLeftAnalogStickDeadZoneUpdated)
					{
						m_leftAnalogStickDeadZone = m_defaultLeftAnalogStickDeadZone;
					}
					m_currentConnectedCount				= ((ScePadVrControllerData*)m_currentData)->connectedCount;
					m_connectionType					= info.connectionType;
				}
			}
		}
		...
		if (logout)
		{
			ret = setScePadDataForUserLogoutCase((ScePadVrControllerData*)m_currentData);
		}
	} else {
		ret = scePadRead(m_handle, (ScePadData *)m_currentData, m_numPadData);
		if (((ScePadData *)m_currentData)->buttons & SCE_PAD_BUTTON_INTERCEPTED)
		{
		...
		} else
		{
			...
			{
				ScePadControllerInformation info;
				int ret2 = scePadGetControllerInformation(m_handle, &info);
				if (ret2 == SCE_HID_ERROR_ALREADY_LOGGED_OUT)
				{
					logout = true;
				} else
				{
					...
					m_defaultRightAnalogStickDeadZone	= (float)(info.stickInfo.deadZoneRight * 2) * m_recipMaxByteAsFloat;
					m_defaultLeftAnalogStickDeadZone	= (float)(info.stickInfo.deadZoneLeft * 2) * m_recipMaxByteAsFloat;
					if (!m_isRightAnalogStickDeadZoneUpdated)
					{
						m_rightAnalogStickDeadZone = m_defaultRightAnalogStickDeadZone;
					}
					...
					m_touchPadWidth						= info.touchPadInfo.resolution.x;
					m_touchPadHeight					= info.touchPadInfo.resolution.y;
					m_touchPadPixelDensity				= (float)info.touchPadInfo.pixelDensity;
					m_touchPadAspectRatio				= (float)m_touchPadWidth / m_touchPadHeight;
	...
	if(m_numCurrentPadData > 0){
		...
		for(int i = 0; i < m_numCurrentPadData; i++) {
			...
			} else if (m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_LEFT || m_type == SCE_PAD_PORT_TYPE_VR_CONTROLLER_RIGHT)
			{
				m_currentPadData[i] = convert(((ScePadVrControllerData *)m_currentData)[m_numCurrentPadData - 1 - i]);
			} else {
...
int ssi::PadContext::setScePadDataForUserLogoutCase(ScePadVrControllerData* data)
{
	memset(data, 0x0, sizeof(ScePadVrControllerData));
	data->connected = false;
	data->analogStick.x = 0x80;
	data->analogStick.y = 0x80;
	data->acceleration.y = 1.0f;
